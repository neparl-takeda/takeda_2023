...
//テクスチャ
ID3D11ShaderResourceView* EntityPBullet::m_Texture{};

void EntityPBullet::Load()
{
	//テクスチャ読み込み
...
void EntityPBullet::Unload()
{
	m_Texture->Release();
	//delete m_Texture;
}

void EntityPBullet::Init()
{
	VERTEX_3D vertex[4];
...
void EntityPBullet::Update()
{
	...
	if (m_Type == TYPE_KUNAI)
	{
		m_Position += m_Direction * 0.5f;
		m_Rotation.z = m_Direction.y;
	}
	else
	if (m_Type == TYPE_SHURIKEN)
	{
		m_Position += m_Direction * 0.5f;
		m_Direction.x *= 0.99f;
		m_Direction.y *= 0.95f;
